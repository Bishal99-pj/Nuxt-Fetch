<template>
  <div
    :id="props.id || 'alert-' + random_id"
    :class="container_classes"
    role="alert"
  >
    <Icon :name="icon" class="shrink-0" size="24" />
    <span class="sr-only">Info</span>
    <div class="ms-3 font-medium capitalize">
      {{ variant }} alert with an
      <a href="#" class="font-semibold underline hover:no-underline"
        >example link</a
      >. Give it a click if you like.
      <div v-if="dismissBtn" class="flex space-x-2 mt-1.5">
        <button
          :data-dismiss-target="'#alert-' + random_id"
          aria-label="Close"
...
onMounted(() => {
  random_id.value = crypto.randomUUID();
});
</script>
