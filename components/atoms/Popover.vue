<template>
    <section class="max-w-sm mx-auto">
        <button
ref="triggerElRef" type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
            @click="popover?.show()">
            Default popover
        </button>
        <div class="flex justify-between">
            <div
id="target-content" ref="targetElRef" data-popover role="tooltip"
                ...
                <span class="flex justify-between px-3 py-2">
                    ...
                    <span
v-if="closeIconVisibility" class="cursor-pointer text-[17px] font-bold"
                        @click.stop="popover?.hide()">x</span>
                </span>
                <div data-popper-arrow :class="twMerge('tooltip-arrow', '!border-0')"/>
            </div>
        </div>
